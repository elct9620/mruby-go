...
func Test_Representation_PoolValue_String(t *testing.T) {
	buffer := bytes.NewReader([]byte{
...
func Test_Representation_PoolValue_Float(t *testing.T) {
	buffer := bytes.NewReader([]byte{
		0x00, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x02, 0x01, 0x00, 0x38,
		0x01, 0x69, 0x00, 0x01, 0x05, 0x1f, 0x85, 0xeb, 0x51, 0xb8, 0x1e, 0x09, 0x40, 0x00, 0x00, 0x45,
	...
	if rep.PoolValue(0) != 3.14 {
		t.Errorf("expected pool value to be 3.14 but got %v", rep.PoolValue(0))
	}
}
